...
const HeroSection = () => {
  ...
  },
  {
    ...
    btnText: "Get Started",
    bg: "https://images.unsplash.com/photo-1624668430039-0175a0fbf006?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTl8fGZyZXNoJTIwdmVnZXRhYmxlc3xlbnwwfHwwfHx8MA%3D%3D",
  },
]
  ...
  return (
    <div className='relative w-[98%] mx-auto mt-32 h-[80vh] rounded-3xl overflow-hidden shadow-2xl'>
        <AnimatePresence mode='wait'>
            ...
            >
                <Image 
                src={slides[current].bg} fill alt='slides' priority
                className='object-cover inset-0 bg-black/50 backdrop-blur-[1px]'/>
            </motion.div>
        </AnimatePresence>
        ...
        <div className='absolute inset-0 flex items-center justify-center text-center text-white px-6'>
            ...
            className='flex items-center justify-center gap-6 max-w-3xl flex-col'
            >
                <div className='bg-white/10 backdrop-blur-md p-6 rounded-full shadow-lg'>
                    {slides[current].icon}
                </div>
                <h1 className='text-3xl md:text-6xl sm:text-5xl font-extrabold tracking-tight drop-shadow-lg '>{slides[current].title}</h1>
                <p className='text-lg md:text-xl text-gray-200 max-w-2xl '>{slides[current].subtitle}</p>
                <motion.button 
