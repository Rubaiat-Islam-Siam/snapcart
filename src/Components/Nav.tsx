...
import { LogOut, Package, Plus, Search, ShoppingCart, User, X,Boxes, ClipboardCheck, Menu } from "lucide-react";
...
const Nav = ({ user }: { user: IUser }) => {
    ...
    <AnimatePresence>
      ...
      >
        <div className="flex justify-between items-center mb-2">
          <h1 className="font-extrabold text-2xl tracking-wide text-white/90">Admin Pannel</h1>
          <button className="text-white/80 hover:text-red-400 text-2xl font-bold transition" onClick={()=> setMenuOpen(false)}><X/></button>
        </div>
        <div className="flex items-center gap-3 mt-3 p-3 rounded-xl bg-white/10 hover:bg-white/15 transition-all shadow-inner">
          ...
          <div>
            ...
            <p className="font-xs text-green-200 capitalize tracking-wide">{user.role}</p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3 font-medium mt-6">
            ...
            <Link href={"/admin/view-grocery"} className="flex items-center gap-3 p-3  bg-white/10 rounded-lg hover:bg-white/20 hover:pl-4 transition-all"><Boxes className='w-5 h-5'/> View Grocery</Link>
            <Link href={"/admin/manage-orders"} className="flex items-center gap-3 p-3  bg-white/10 rounded-lg hover:bg-white/20 hover:pl-4 transition-all"><ClipboardCheck className='w-5 h-5'/> Manage Orders</Link>
          </div>
  ...
  return (
    <div className="w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-18 px-4 md:px-8 z-50">
      <Link
        href="/"
